package com.faunjoe.spring;



import com.faunjoe.annotation.ExtResource;
import com.faunjoe.entity.BeanDefine;
...
public class ClassPathXmlApplicationContext {
    ...
    /**
     * 注解处理器
     * 如果注解ExtResource配置了name属性，则根据name所指定的名称获取要注入的实例引用，
     * 如果注解ExtResource;没有配置name属性，则根据属性所属类型来扫描配置文件获取要
     * 注入的实例引用
     */
    ...
    public void propertyAnnotation(Object bean) {
        try {
            ...
            for (PropertyDescriptor proderdesc : ps) {
                ...
                //判断set方法是否定义了注解
                if (setter != null && setter.isAnnotationPresent(ExtResource.class)) {
                    //获取当前注解，并判断name属性是否为空
                    ExtResource resource = setter.getAnnotation(ExtResource.class);
                    String name = "";
                    Object value = null;
    ...
    public void fieldAnnotation(Object bean) {
        try {
            ...
            for (Field f : fields) {
                if (f != null && f.isAnnotationPresent(ExtResource.class)) {
                    ExtResource resource = f.getAnnotation(ExtResource.class);
                    String name = "";
                    Object value = null;
                    if (resource.name() != null && !"".equals(resource.name())) {
                        name = resource.name();
