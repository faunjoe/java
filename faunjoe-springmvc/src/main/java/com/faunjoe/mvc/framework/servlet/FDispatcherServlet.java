package com.faunjoe.mvc.framework.servlet;
...
import com.faunjoe.mvc.framework.context.FApplicationContext;
...
public class FDispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		//IOC容器必须要先初始化
		//假装容器已启动
		FApplicationContext context = new FApplicationContext(config.getInitParameter(LOCATION));
...
//		Map<String, Object> ioc = context.getAll();
	 ...
	 * @param context
	 */
	private void initMultipartResolver(FApplicationContext context){}

	/**
	 * 多语言、国际化
	 * @param context
	 */
	private void initLocaleResolver(FApplicationContext context){}

	/**
	 * 主题View层的
	 * @param context
	 */
	private void initThemeResolver(FApplicationContext context){}

	/**
	 * 解析url和Method的关联关系
	 * @param context
	 */
	private void initHandlerMappings(FApplicationContext context){
		Map<String,Object> ioc = context.getAll();
	 ...
	 * @param context
	 */
	private void initHandlerAdapters(FApplicationContext context){
			...
			for (int i = 0; i < pa.length; i ++) {
				for(Annotation a : pa[i]){
					if(a instanceof FRequestParam){
						String paramName = ((FRequestParam) a).value();
	 ...
	 * @param context
	 */
	private void initHandlerExceptionResolvers(FApplicationContext context){}

	/**
	 * 视图转发（根据视图名字匹配到一个具体模板）
	 * @param context
	 */
	private void initRequestToViewNameTranslator(FApplicationContext context){}

	/**
	 * 解析模板中的内容（拿到服务器传过来的数据，生成HTML代码）
	 * @param context
	 */
	private void initViewResolvers(FApplicationContext context){
	...
	private void initFlashMapManager(FApplicationContext context){}
	...
	private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception{
		...
		try{
			...
			FModelAndView mv = ha.handle(req, resp, handler);
	...
	public void applyDefaultViewName(HttpServletResponse resp,FModelAndView mv) throws Exception{
		if(null == mv){ return;}
	...
	private class HandlerAdapter{
		...
		public FModelAndView handle(HttpServletRequest req, HttpServletResponse resp,Handler handler) throws Exception{
			...
			boolean isModelAndView = handler.method.getReturnType() == FModelAndView.class;
			Object r = handler.method.invoke(handler.controller, paramValues);
			if(isModelAndView){
				return (FModelAndView)r;
			}else{
				return null;
			}
	...
	private class ViewResolver{
		...
		protected String parse(FModelAndView mv) throws Exception{
